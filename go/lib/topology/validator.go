...
// ColibriValidator is the validator that should be used for colibri services.
type ColibriValidator struct {
	ID       string
	v        *validatorWrap
	initOnce sync.Once
}

func (v *ColibriValidator) Validate(new, old *RWTopology) error {
	v.init()
	return v.v.Validate(new, old)
}

func (v *ColibriValidator) init() {
	v.initOnce.Do(func() {
		v.v = &validatorWrap{&svcValidator{id: v.ID, svc: Colibri}}
	})
}
