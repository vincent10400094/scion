...
func (l *Loader) ColibriServiceAddresses() []*net.UDPAddr {
	l.mtx.Lock()
	defer l.mtx.Unlock()

	addrs, err := l.topo.MakeHostInfos(Colibri)
	if err != nil {
		return nil // should only happen on empty addrs.
	}
	return addrs
}

func (l *Loader) ColibriServiceAddress(id string) *net.UDPAddr {
	l.mtx.Lock()
	defer l.mtx.Unlock()
